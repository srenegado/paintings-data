...
class TestWork(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        print("Validating work table...")
        cls.engine = get_db_engine()
        cls.con = get_db_connection(engine=cls.engine)
        cls.df = pd.read_sql("SELECT * FROM work", con=cls.con)
    ...
    def test_artist_id_references_artist(self):
        ...
        invalid_artist_id_counts = self.df[
            ~self.df["artist_id"].isin(artist_ids["id"])
        ].shape[0]
    ...
    def test_museum_id_references_museum(self):
        ...
        invalid_museum_id_counts = self.df[
            ~self.df["museum_id"].isin(museum_ids["id"])
        ].shape[0]
